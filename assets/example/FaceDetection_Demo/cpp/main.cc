...
#define MODEL_PATH "./models/face_detection.bin"
...
int main()
{
    ...
    // 新增：打开视频流
    cv::VideoCapture cap("/dev/video0"); // 0为摄像头，或替换为视频文件路径
    if (!cap.isOpened()) {
    ...
    while (cap.read(img)) {
        ...
        std::cout << "frame (cols, rows, channels): (";
        std::cout << img.rows << ", ";
        std::cout << img.cols << ", ";
        ...
        if (PREPROCESS_TYPE == LETTERBOX_TYPE) // letter box
        {
            ...
            elapsed_ms = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now() - begin_time).count() / 1000.0; 
        }
        ...
        elapsed_ms = elapsed_ms + std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now() - begin_time).count() / 1000.0;
