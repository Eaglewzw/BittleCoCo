...
namespace OpenCat
{
    ...
    void Robot::SendMotorCommand( std::string motor_command, bool verbose)
    {

        using std::chrono::milliseconds;

        string cmd_name = motor_command;
        ...
        if (verbose)
        {
            std::cout << "Start executing: " << motor_command
                    << std::endl;
        }
        
        if (this->send(data) < 0) {
            throw std::runtime_error("Failed to send motor command");
        }
